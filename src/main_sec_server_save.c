...
static mmt_sec_handler_t *mmt_sec_handler[10];
...
int main(int argc, char** argv) {
	...
	for (i=0; i<nbr_thr_p; i++){
		mmt_sec_handler[i] = mmt_sec_register( rules_arr, size, print_verdict, NULL );	//register mmt_sec corresponding to rules here
		size += mmt_sec_get_unique_protocol_attributes( mmt_sec_handler[i], &p_atts );
	}
	...
	/*
	  ...
	  for (i=0; i<nbr_thr_p; i++){
		  mmt_sec_unregister( mmt_sec_handler[i]);
	  	  }
