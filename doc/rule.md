...
1. `is_exist( proto.att )`  checks whether an event has an attribute of a protocol, e.g., `is_exist( http.method )` will return `true` if the current event contains protocol `HTTP` and attribute method has a non-null value, otherwise it will return `false`.
	...
	Normally MMT has a filter that allow an event in a rule to be verified only if any proto.att used in its boolean expression contains value. If one of them has not, the rule will not be verified. This allow to reduce number of verification of boolean expression, thus increase the performance.
...
MMT-Security rules are specified in plain text following an XML format.
...
- use `make compile_rule` to compile this programe to get an executable program `compile_rule`

- use `compile_rule` to compile rules in an XML file. For example:

   `./compile_rule rules/1.so rules/1.ssh.xml`
   
   The program uses 3 paramters in form: `./compile_rule output_file property_file [gcc parameters]`
...
## 3.2 Obtain information on the compiled rules 
...
- use `./rule_info` to print information of all compiled rules in `./rules` or `/opt/mmt/security/rules/`
   ...
   By default, the tool will print out the information on all rules, for instance:
...
# 4. Write a high performance rule
