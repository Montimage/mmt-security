...
static bool notdone, recv_state[10]; //use notdone to terminate the server,  receiving state =0 if no connection
static int connectcnt; //counter of connections
...
struct {
        ...
        pthread_spinlock_t spinlock_flag; //lock for flag concerning each report
...
int insert(report_t **head, report_t *report_node)
{
	...
	current = (report_t *) mmt_mem_alloc(sizeof(report_t));
...
void *receiving_thr (void *arg) {
	...
	while(1){
		...
		if ((int) pthread_spin_lock(&thread_lock.spinlock_cr)) error("spinlock_cr failed");
...
/*
int sizeof_report_list(report_t *report_list){ //TODO: find another way to delay, this repeated loop will kill the performance
	int size;
	report_t *last_node = report_list;
	size = (int) (sizeof(report_element_t) * last_node->elements_count +  sizeof(report_t));
	while(last_node->next!=NULL){
				last_node = last_node->next;
				size += sizeof(report_t) + sizeof(report_element_t) * last_node->elements_count;
			} //move to last_node
	return size;
}
*/
...
bool delay_cond(report_t *head){
if (!receiving_state()) return NO;
if (head->prev==NULL) return YES; //only one node in the list
if ((thread_lock.count_str < THRESHOLD_SIZE) && ((head->timestamp.tv_sec - head->prev->timestamp.tv_sec) < THRESHOLD_TS)) {
	return YES; //still delay because the server is receiving new report
	}
return NO;
}
...
while(notdone)
	{
	//mmt_debug("Processing threads locked");
	...
	//mmt_debug("Processing threads unlock");
...
int main(int argc, char** argv) {
	...
	on = 1;
	notdone = YES;
	connectcnt = 0; //nbr of connections
	portno = 5001;
	pthread_t thr_r[3];
	pthread_t thr_p[3];
	...
	pthread_spin_init(&thread_lock.spinlock_flag, 0);
	...
	for (i=0; i<nbr_thr_p; i++){
			if (pthread_create(&thr_p[nbr_thr_p], NULL, processing_thr,(void*) mmt_sec_handler)) error("Can't create threads for processing");
			}
	...
	/*
	  ...
	  while (notdone) {
	    ...
	    if (pthread_create(&thr_r[connectcnt], NULL, receiving_thr,(void*) (void*) &thr_recv_arg)) error("Can't create threads for reading");
		connectcnt++;
	    }
	  }
