...
void _iterate_to_print( void *key, void *data, void *user_data, size_t index, size_t total ){
	printf("ev_id_%d = %p\n", *(uint16_t *)key, data);
}
...
static inline void _store_valid_execution_trace( _rule_engine_t *_engine, fsm_t *fsm ){
	...
	mmt_map_iterate( _engine->valid_execution_trace, _iterate_to_print, NULL );
