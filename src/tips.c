...
rule_t * create_rule()
{
    rule_t * a_rule = (rule_t *) xmalloc(sizeof (rule_t));
...
void create_father(rule_t *a_rule, short depth, short clean)
{
    father_t *temp = top_father;
    if (clean == CLEAN) {
        while (temp) {
    ...
    if (top_father == NULL) {
        top_father = (father_t *) xmalloc(sizeof (father_t));
        ...
        temp = (father_t *) xmalloc(sizeof (father_t));
        temp->node = a_rule;
        temp->depth = depth;
        temp->next = NULL;
...
char * get_value( const ipacket_t *pkt, char *input, short *jump, short *size, tuple_t *list_of_tuples )
{
    int i = 0;
    char * output = NULL;
    char * temp2 = NULL;
...
void register_tuple(mmt_handler_t *mmt, tuple_t * a_tuple)
{
    int ret = 1;
...
void add_tuple_to_list_of_tuples(tuple_t *a_tuple)
{
    if (a_tuple->event_id > 0) {
        tuple_t * b_tuple = (tuple_t *) xmalloc(sizeof (tuple_t));
        ...
        b_tuple->data = NULL;
        b_tuple->next = NULL;
        tuple_t *temp_tuple = root_rule->list_of_tuples; //add it to the ROOT (i.e. root_rule)
        int found = NO;
...
char * funct_get_info_param( mmt_handler_t *mmt, short reg_tuple, char * input, tuple_t *a_tuple)
{
...
void * funct_get_params_and_execute( const ipacket_t *pkt, short skip_refs, char *lib_name, char *funct_name, int data_size, tuple_t *tt, tuple_t *list_of_tuples, short *found )
{
...
void create_boolean_expression(mmt_handler_t *mmt, int first_time, rule_t *a_rule, char *expression)
{
    ...
    if (*temp == '(') {
        ...
        if (first_time == YES) {
            ...
            if (a_rule->list_of_sons == NULL) {
                ...
                rule_t *a_temp = a_rule->list_of_sons;
        ...
        if (ref > 0) {
            ...
            tuple_t * a_tuple = (tuple_t *) xmalloc(sizeof (tuple_t));
            ...
            a_tuple->data = NULL;
            a_tuple->next = NULL;
            tuple_t *temp_tuple = top_rule->list_of_tuples;
            int found = NO;
            while (temp_tuple != NULL) {
        ...
        char * command2;
        tuple_t * top_tuple = (tuple_t *) xmalloc(sizeof (tuple_t)); //top of parameter list
        ...
        while( command2 ) {
            new_tuple = (tuple_t *)xmalloc(sizeof (tuple_t));
...
void recuperate_attributes(rule_t* root, rule_t *r)
{
    rule_t *s = NULL;
    int found = NO;
    tuple_t *temp_tuple = NULL;
    tuple_t * a_tuple = NULL;
    if (r->t.protocol_id != 0 && r->t.field_id != 0) {
        ...
        a_tuple = (tuple_t *) xmalloc(sizeof (tuple_t));
...
rule_t *create_instance(rule_t **root_inst, rule_t *r, rule_t* father)
{
    rule_t *a_rule = create_rule();
    rule_t *temp_inst = NULL;
    rule_t *temp_sons = r->list_of_sons;
    rule_t *temp_next = r->next;
    rule_t *temp = NULL;
    tuple_t *temp_tuple = r->list_of_tuples;
...
void set_events_to_not_yet(int buff_ids[100], rule_t * r, rule_t *orig_rule)
{
  rule_t *pt = r;
  rule_t *pt_orig = orig_rule;
  short all_valid = YES;
  while(pt != NULL){
    ...
    if(pt->event_id >0){
      if (event_found(buff_ids, pt->event_id) == NO){
        ...
        tuple_t *temp_tuple = &(pt->t);
        tuple_t *temp_tuple_orig = &(pt_orig->t);
        while(temp_tuple != NULL){
...
rule_t *copy_instance(rule_t **root_inst, rule_t *r, rule_t* father, rule_t *orig_rule)
{
  char *pt = NULL, *pt2 = NULL, *pt3 = NULL;
  pt = orig_rule->keep_state;
  if(pt == NULL) return NULL;
  rule_t *new_rule = create_instance(root_inst, r, father);
  ...
  if(new_rule != NULL){
    ...
    rule_t * temp_rule = new_rule->list_of_sons;
    rule_t * temp_orig = orig_rule->list_of_sons;
...
int eliminate_instance(rule_t **r, rule_t **i, char *type)
{
    ...
    if ((*i)->list_of_sons != NULL) {
        rule_t * temp_rule = (*i)->list_of_sons;
        rule_t *temp_r = NULL;
        rule_t *temp_rr = *i;
    ...
    if ((*i)->list_of_tuples != NULL) {
        tuple_t *temp_tuple = (*i)->list_of_tuples;
        tuple_t *temp_t = NULL;
        while (temp_tuple != NULL) {
...
void store_history(const ipacket_t *pkt, short context, rule_t *curr_root, rule_t *curr_rule, char *cause, short event_id)
{
    unsigned long L1=0,L2=0,L3=0,L4=0;
    ...
    if (*cause != '\0') {
        ...
        tuple_t * temp = curr_root->list_of_tuples_to_print;
        while (temp) {
...
void store_tuples( const ipacket_t *pkt, short context, rule_t *curr_root, rule_t *curr_rule, short event_id, char *cause )
{
    tuple_t * temp_tuple = NULL;
...
int get_data_from_pcap( const ipacket_t *pkt, short skip_refs, short action, void** result_value, tuple_t *list_of_tuples, short operator, rule_t *r1, rule_t *r2)
{
    int ret = 0;
    tuple_t *temp_tuple = list_of_tuples;
    tuple_t *temp_tuple2 = NULL;
    compare_value v1;
    compare_value v2;
    compare_value *tmp_v;
...
void detected_corrupted_message(short print_option, rule_t *r, char *cause, short state, struct timeval packet_time_stamp)
{
    rule_t *temp = r;
    char *history;
...
int verify_segment( const ipacket_t *pkt, short skip_refs, tuple_t *list_of_tuples, rule_t *c, rule_t *curr_root )
{
    short result = 0;
...
void print_nothing(short print_option, rule_t *curr_root, rule_t *r, char *cause, short state) {
    //(void)fprintf(stderr, "nothing\n");
}

char *generate_command( const ipacket_t *pkt, tuple_t *list_of_tuples, char * input )
{
...
void rule_is_satisfied_or_not(const ipacket_t *pkt, short print_option, rule_t *curr_root, rule_t *r, char *cause, short state, short use_cause) {
    short result = 0;
    char *command = NULL;
    if (r->description == NULL) {
    ...
    if (do_it == 1) {
        if (what_to_do != NULL) {
            if (strchr(what_to_do, '#') == NULL) {
                ...
                while (*command == ' ') command++;
                char * command2;
                tuple_t * top_tuple = (tuple_t *) xmalloc(sizeof (tuple_t));
                top_tuple->protocol_id = -1;
                top_tuple->field_id = -1;
                top_tuple->data_type_id = -1;
                top_tuple->data_size = -1;
...
int check_for_countout(rule_t *r, int count)
{
...
int check_for_timeout(rule_t *r, struct timeval start, struct timeval curr)
{
    int ret = 0;
...
short check_time(rule_t *r, struct timeval curr)
{
    short result = SKIP2;
...
short action(short situation, short leftleft, rule_t *r)
{
...
int verify_left(const ipacket_t *pkt, char *cause, rule_t *r, rule_t *root)
{
    short result = 0;
...
int verify( const ipacket_t *pkt, short leftleft, short context, rule_t *curr_root, tuple_t *list_of_tuples, short *reference,
        char *cause, rule_t *r, struct timeval current_packet_time)
{
    short result = 0, situation = 0, this = 0;
    rule_t *curr_r = NULL;
    rule_t *curr_r_NOT = NULL;
    rule_t *temp_rule = NULL;
    *cause = '\0';
    switch (r->value) {
        case THEN:
...
int analyse_incoming_packet(const ipacket_t * ipacket, void* arg)
{
    ...
    short result = 0;
    rule_t *curr_rule = top_rule;
    rule_t *curr_rule_instance = NULL;
    rule_t *temp = NULL;
    rule_t *root_inst = NULL;
