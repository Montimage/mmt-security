...
#define mmt_error( ... ) mmt_sec_log( ERROR, __VA_ARGS__ )
	...
	#define mmt_assert( expr, ... ) while( unlikely( !(expr) ) ){ printf("%s:%d ", __FILE__, __LINE__); mmt_sec_log( HALT, __VA_ARGS__ ); }
#else
	#define EXEC_ONLY_IN_DEBUG_MODE( expr )
	#define mmt_debug(...)
	#define mmt_halt( ... ) mmt_sec_log( HALT, __VA_ARGS__ )
	#define mmt_assert( expr, ... ) while( unlikely( !(expr) ) ){ mmt_sec_log( HALT, __VA_ARGS__ ); }
#endif
