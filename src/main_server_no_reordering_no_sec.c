/*
 * main_server_no_reordering_no_sec.c
 * No reordering, no sec, receive report then free
...
void *receiving_thr (void *arg) {
	...
	while(1){
			...
			length += 4;
			memcpy(&report_node->report_elements[counter]a.data_len,&buffer[length],2);
			length += 2;
			...
			//report_handler(mmt_smp_sec_handler, report_node);
			free_report_t(report_node);
